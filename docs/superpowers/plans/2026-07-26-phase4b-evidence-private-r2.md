...
- **Design source of truth:** `docs/superpowers/specs/2026-07-26-phase4b-evidence-private-r2-design.md`. Read it before starting.
...
- **Schema is table-only for business logic** (no *business* DB functions/triggers). `updated_at` is maintained by the common `set_updated_at()` trigger — every new table with `updated_at` gets the trigger; Go SQL must NOT write `updated_at`. Use the pinned Atlas (Standard distribution, no login).
...
- Atlas schema dir — `evidences`, `evidence_assets`, `notification_sent_log` tables + triggers.
...
**Interfaces:**
- Produces (DB): tables `public.evidences`, `public.evidence_assets`, `public.notification_sent_log`; each of the first two with a `set_updated_at` trigger.

- [ ] **Step 1: Write the failing integration test asserting the tables + trigger exist**
...
func TestSchema_EvidenceTablesAndTriggers(t *testing.T) {
	...
	// updated_at trigger present on the two mutable tables
	for _, tbl := range []string{"evidences", "evidence_assets"} {
		var n int
		err := db.QueryRowContext(ctx,
			`SELECT count(*) FROM pg_trigger t
			 JOIN pg_class c ON c.oid=t.tgrelid
			 WHERE c.relname=$1 AND NOT t.tgisinternal
			   AND t.tgname LIKE '%updated_at%'`, tbl).Scan(&n)
		if err != nil {
			t.Fatalf("trigger query %s: %v", tbl, err)
		}
		if n == 0 {
			t.Fatalf("expected set_updated_at trigger on public.%s", tbl)
		}
	}
}
```
...
Run: `DATABASE_URL=... go test ./internal/evidence/ -run TestSchema_EvidenceTablesAndTriggers -v`
Expected: FAIL (tables do not exist).

- [ ] **Step 3: Add the Atlas schema** (table-only; `updated_at` via the common trigger)
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.evidences
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.evidence_assets
    FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
- Produces `Store` with (all take `database.Querier` so they compose in a tx):
  ...
  - `CloseRefereeRequestInTx(ctx, q, requestID uuid.UUID) error` — `UPDATE referee_requests SET status='closed' WHERE id=$1`.
  ...
  - `MarkAssetPurgedInTx(ctx, q, assetID uuid.UUID) error` — set `purged_at = now()`.
