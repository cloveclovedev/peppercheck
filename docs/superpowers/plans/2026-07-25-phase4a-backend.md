...
- **Design source of truth:** `docs/superpowers/specs/2026-07-25-phase4a-task-authoring-matching-design.md`. Follow-ups: `docs/superpowers/specs/2026-07-25-phase4a-follow-ups.md`.
- **Schema is table-only for business logic** (no *business* DB functions/triggers). `updated_at` is maintained by the common `set_updated_at()` trigger introduced in Phase 3a (P3a-D11) — every new table with `updated_at` gets the trigger; Go SQL must NOT write `updated_at`. Atlas is pinned; use the pinned Atlas (Standard distribution, no login).
...
> **`updated_at` triggers (P4a-D15):** attach the Phase 3a common `set_updated_at()`
> trigger to every table above that has an `updated_at` column (`tasks`,
> `referee_requests`, `judgements`, `referee_available_time_slots`,
> `referee_blocked_dates`, `referee_availability`, `matching_config`). Follow the exact per-table trigger
> definition 3a established; do NOT set `updated_at` from Go. The 3a DB assertion
> ("`updated_at` column ⇒ trigger") will cover these tables.
...
// expires + refunds it. `updated_at` is bumped by the set_updated_at trigger. A
...
> Add `SetPointSourceInTx(ctx, tx, requestID uuid.UUID, source string) error` to the matching store (`UPDATE public.referee_requests SET point_source = $2 WHERE id = $1`; `updated_at` bumped by trigger). The no-op locker returns `"regular"`, so this is a no-op stamp in 4a.
...
- **Re-review round 1 fixes applied:** open-deadline validation now uses `matching_config.open_deadline_hours` and the real seed values (24/12/14) (T2/T11); the same-task 23505 propagates out of the tx and is mapped to success outside (T8) with a same-task concurrency test; the sweep expire is a CAS in the refund/notify tx with an accept-vs-expire test (T9); the recurring sweep self-reschedules with a bucketed idempotency key (T9/T12); cancel carries `point_source` with a trial-preservation test (T10); `judgements` added to the `updated_at` trigger list (T2); the worker's `FIREBASE_PROJECT_ID` + SA secret land in prod Compose (T12 Step 6).
...
- **Known execution-time confirmations (not placeholders — verified against real docs/3a at execution):** 3a symbol names (`identity.CurrentUserFrom`, `database.Querier`, `notification` store shape, the `set_updated_at` trigger definition); FCM SDK loc-key field paths + pgx array binding (official docs). Each is flagged inline at its task.
