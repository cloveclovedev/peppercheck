...
> `docs/designs/2026-07-22-supabase-to-go-vps-refactor-design.md`, the
> Phase 0 baseline `docs/designs/2026-07-22-phase0-baseline.md`, and the
> Phase 4a spec `docs/designs/2026-07-25-phase4a-task-authoring-matching-design.md`.
...
`updated_at` is maintained by explicit Store SQL per P3a-D11 / P4a-D15).
Clearly-poor implementations may be improved during the Go
...
- Phase 3a — `platform/r2` presigned-PUT adapter (`PresignPut` / `Head` /
  ...
  `send_notification` outbox/worker foundation groundwork; Go-maintained
...
maintained by explicit Store `UPDATE` and `ON CONFLICT DO UPDATE` assignments
(P3a-D11, P4a-D15); inserts use `DEFAULT now()`. FKs target the Go identity core
...
evidences                          (was task_evidences)
  ...
  updated_at   timestamptz NOT NULL DEFAULT now()      -- Store SQL updates explicitly
...
- **Postgres integration** (real container): migrations apply to empty DB;
  constraints/indexes/FKs; Store update/upsert coverage for every new table
  with `updated_at`; submit atomicity (evidence + assets + all judgements→in_review +
