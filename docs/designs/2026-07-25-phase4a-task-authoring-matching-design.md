...
> `docs/designs/2026-07-22-supabase-to-go-vps-refactor-design.md` and
> the Phase 0 baseline `docs/designs/2026-07-22-phase0-baseline.md`.
...
*business* DB functions/triggers — Store SQL explicitly maintains `updated_at`
per Phase 3a P3a-D11). Clearly-poor implementations may
...
| **P4a-D15** | **`updated_at` is maintained explicitly by Go-owned Store SQL** (Phase 3a P3a-D11): mutable `UPDATE` and `ON CONFLICT DO UPDATE` statements set `updated_at = now()`; inserts use the column default. | Keeps the target schema free of functions and triggers while Go remains the sole write boundary. PostgreSQL still provides the transaction timestamp. |
...
Atlas table-only with no functions or triggers initially. `updated_at` is
maintained by explicit Store SQL (P3a-D11, P4a-D15); every mutable table below
carries the column and each mutation path updates it. FKs target the Go identity core
...
- **Postgres integration** (real container): migrations apply to empty DB;
  ...
  same-task partial unique index, Store update/upsert coverage for every
  `updated_at` column); publish atomicity (task open + requests + jobs all-or-nothing;
...
See `docs/designs/2026-07-25-phase4a-follow-ups.md`.
