...
-- email/email_verified: captured from the verified token at provisioning and
-- refreshed on every login (Store.TouchIdentityEmail). Used by Phase 3b's
-- unauthenticated account-deletion request resource (FindUserByEmail) to
-- match a claimed email to a real account -- only verified rows ever match.
