...
#
# For production, the Firebase project (peppercheck) and both Android/iOS
# app entries pre-exist; the script will skip apps:create and only
# re-download configs (useful for re-onboarding a workstation).
...
env="${1:-}"
case "$env" in
  dev)        project_id="peppercheck-dev";     suffix=".dev";     cap_env="Dev" ;;
  staging)    project_id="peppercheck-staging"; suffix=".staging"; cap_env="Staging" ;;
  production) project_id="peppercheck";         suffix="";         cap_env="Production" ;;
...
# --- Android ---
android_app_id="$(echo "$apps_json" | jq -r ".result[] | select(.platform == \"ANDROID\" and (.packageName // \"\") == \"$bundle_id\") | .appId" | head -n1)"
...
# --- iOS ---
ios_app_id="$(echo "$apps_json" | jq -r ".result[] | select(.platform == \"IOS\" and (.bundleId // \"\") == \"$bundle_id\") | .appId" | head -n1)"
