-- Function to reset points for subscription renewal idempotently
-- Sets balance to p_amount + locked (preserving in-flight match locks)
-- Returns true if points were reset, false if already processed based on invoice_id
CREATE OR REPLACE FUNCTION public.reset_subscription_points(
    p_user_id uuid,
    p_amount integer,
    p_invoice_id text
)
...
ALTER FUNCTION public.reset_subscription_points(uuid, integer, text) OWNER TO postgres;
